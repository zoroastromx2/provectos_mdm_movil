...
set(QGIS_SHA512
    6eb3e2259fda72f7e2e8203d951b891963d52d47128793af02dfa989a467ba5524ab7a1019b4cf4acfd9e70c21635bd7b8577dc452d5cf7b98fcf7ce03d97b9f
)
...
list(APPEND QGIS_OPTIONS -DWITH_GUI=ON)
...
# QGIS likes to install auth and providers to different locations on each platform let's
# keep things clean and tidy and put them at a predictable location
list(APPEND QGIS_OPTIONS -DQGIS_PLUGIN_SUBDIR=lib)

# By default QGIS installs includes into "include" on Windows and into "include/qgis"
# everywhere else let's keep things clean and tidy and put them at a predictable location
list(APPEND QGIS_OPTIONS -DQGIS_INCLUDE_SUBDIR=include/qgis)

if (VCPKG_TARGET_IS_WINDOWS)
  # ######################################################################################
  # Install pip if(NOT EXISTS "${PYTHON3_PATH}/Scripts/pip.exe") MESSAGE(STATUS  "Install
  # pip for Python Begin ...") vcpkg_download_distfile( GET_PIP_PATH URLS
  # https://bootstrap.pypa.io/pip/3.5/get-pip.py FILENAME get-pip.py SHA512
  # a408dd74e69a2c3259e06669562bebfff676156a1a3b2f8ba6594ed308e3f94afdf79257688e59b02bf02bd7f6ecbea510fc6665b403149031f14bcc08308a47
  # )

  # vcpkg_execute_required_process( COMMAND "${PYTHON_EXECUTABLE}" "${GET_PIP_PATH}"
  # WORKING_DIRECTORY ${PYTHON3_PATH} LOGNAME pip )

  # vcpkg_execute_required_process( COMMAND "${PYTHON_EXECUTABLE}" -m pip install
  # --upgrade pip WORKING_DIRECTORY ${PYTHON3_PATH} LOGNAME pip ) MESSAGE(STATUS  "Install
  # pip for Python End") endif (NOT EXISTS "${PYTHON3_PATH}/Scripts/pip.exe")
  # ######################################################################################
  ...
  # qgis_gui depends on Qt5UiTools, and Qt5UiTools is a static library. If
  # Qt5_EXCLUDE_STATIC_DEPENDENCIES is not set, it will add the QT release library that it
  # depends on. so that in debug mode, it will reference both the qt debug library and the
  # release library. In Debug mode, add Qt5_EXCLUDE_STATIC_DEPENDENCIES to avoid this bug
  ...
  # FIND_LIB_OPTIONS(PYTHON python39 python39_d LIBRARY
  # ${VCPKG_TARGET_IMPORT_LIBRARY_SUFFIX})
  find_lib_options(
    QCA
    qca
  ...
  # FIND_LIB_OPTIONS(QSCINTILLA qscintilla2_qt5 qscintilla2_qt5d LIBRARY
  # ${VCPKG_TARGET_IMPORT_LIBRARY_SUFFIX})

  # if (VCPKG_LIBRARY_LINKAGE STREQUAL "static") FIND_LIB_OPTIONS(ZSTD zstd_static
  # zstd_staticd LIBRARY ${VCPKG_TARGET_IMPORT_LIBRARY_SUFFIX}) endif()

  set(SPATIALINDEX_LIB_NAME spatialindex)
  if (VCPKG_TARGET_ARCHITECTURE STREQUAL "x64" OR VCPKG_TARGET_ARCHITECTURE STREQUAL
                                                  "arm64"
  )
  ...
  find_lib_options(
    SPATIALINDEX
    ${SPATIALINDEX_LIB_NAME}
    ${SPATIALINDEX_LIB_NAME}d
    LIBRARY
    ${VCPKG_TARGET_IMPORT_LIBRARY_SUFFIX}
  )
endif ()
...
function (copy_path basepath targetdir)
  ...
  if (${basepath}_PATH)
    ...
    file(COPY ${${basepath}_PATH}
         DESTINATION ${CURRENT_PACKAGES_DIR}/${targetdir}/${PORT}/${basepath}
    )
  endif ()
...
if (VCPKG_TARGET_IS_WINDOWS)
  file(
    ...
    ${CURRENT_PACKAGES_DIR}/*${VCPKG_TARGET_EXECUTABLE_SUFFIX}
  )
  if (QGIS_TOOL_PATH)
  ...
  if (QGIS_TOOL_PATH_DEBUG)
    if ("debug-tools" IN_LIST FEATURES)
      ...
      file(COPY ${QGIS_TOOL_PATH_DEBUG}
           DESTINATION ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/bin
      )
  ...
  # Extend vcpkg_copy_tool_dependencies to support the export of dll and exe dependencies
  # in different directories to the same directory, and support the copy of debug
  # dependencies
  function (
    vcpkg_copy_tool_dependencies_ex
    TOOL_DIR
    OUTPUT_DIR
    SEARCH_DIR
  )
    ...
    if (PS_EXE-NOTFOUND)
      message(
        FATAL_ERROR
          "Could not find powershell in vcpkg tools, please open an issue to report this."
      )
    endif ()
    macro (search_for_dependencies PATH_TO_SEARCH)
      ...
      foreach (TOOL ${TOOLS})
        vcpkg_execute_required_process(
          ...
          -nologo
          -file
          ${CMAKE_CURRENT_LIST_DIR}/applocal.ps1
          -targetBinary
          ${TOOL}
          -installedDir
          ${PATH_TO_SEARCH}
  ...
  vcpkg_copy_tool_dependencies_ex(
    ${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin ${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin
    bin
  )
  vcpkg_copy_tool_dependencies_ex(
    ${CURRENT_PACKAGES_DIR}/tools/${PORT}/plugins
    ${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin bin
  )
  if ("debug-tools" IN_LIST FEATURES)
    vcpkg_copy_tool_dependencies_ex(
      ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/bin
      ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/bin debug/bin
    )
    vcpkg_copy_tool_dependencies_ex(
      ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/plugins
      ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/bin debug/bin
    )
  endif ()
  if ("server" IN_LIST FEATURES)
    vcpkg_copy_tool_dependencies_ex(
      ${CURRENT_PACKAGES_DIR}/tools/${PORT}/server
      ${CURRENT_PACKAGES_DIR}/tools/${PORT}/bin bin
    )
    if ("debug-tools" IN_LIST FEATURES)
      vcpkg_copy_tool_dependencies_ex(
        ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/server
        ${CURRENT_PACKAGES_DIR}/debug/tools/${PORT}/bin debug/bin
      )
    endif ()
  endif ()
